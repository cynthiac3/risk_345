...
vector<Country*> Graph::getCountry(){
	vector<Country*> tmpCountry;
	vector<string> tmp_nbr;
	int index =0;

	for (vector<vertex*>::iterator i = nodes.begin() ; i != nodes.end(); ++i){
		tmpCountry.push_back(new Country());// creats a new vertex
	...
	for (vector<vertex*>::iterator i = nodes.begin() ; i != nodes.end(); ++i){
		...
		for (vector<Country*>::iterator j = tmpCountry.begin() ; j != tmpCountry.end(); ++j){
			for (vector<string>::iterator r = tmp_nbr.begin() ; r != tmp_nbr.end(); ++r){
        			if((*r).compare((*j)->name) == 0){
					tmpCountry[index]->nbr.push_back(*j);
				}
			}
		}
		index++;
...
//return a vertex with macthing name
...
//return vector of neighbours vertex
...
void Graph::printGraph(){
	...
	for (vector<vertex*>::iterator i = nodes.begin() ; i != nodes.end(); ++i){
		cout << "\tTERITORY: " << (*i)->name << " Owner:" << (*i)->owner->getName() << " Armies:"<< (*i)->nbOfArmies <<" NEIGHBOURS:" << endl; // print the country
...
//steal country  (for testing)
void Graph::steal(Player *t,Player *v){
	for (vector<vertex*>::iterator i = nodes.begin() ; i != nodes.end(); ++i){
		if((*i)->owner == v){
			(*i)->owner = t;	
		}
